...
uint8_t CalRTC_REG(uint32_t PCLK,int8_t mode,float tmpval) 
{
  ...
  int16_t PPMx10,ACP4;
  float Ti;
  uint32_t cal_value, div_value, ACP5,ACP6;
  uint8_t cnt=0;
  /*RmvCodCmntA_K*/ 
...
//    ktemp4 = (int8_t)((RTC->ACKTEMP >> 24) & 0xFF);
    ...
    Ti = RTC->ACTI / 256.0;
        /* Get ACK(K parameter) */
        /* we can used parameters that send by SD612 and get it by access addresses or calculate it
        address --> KK1 =*(K1);
        by calculation --> parapolic cofficient with ppm * 10^13 NOTE :PPM=10^-6
        ex: (0.036 * 10^-6 * 10^13)/1000000 * 65536 =23592  */
        ack_used= 23592; //0.035*10-6
...
//      ack_used = RTC->ACK5;
    ...
    // ACP4 = RTC->ACP4;
     ACP4 = *(P4); //offset
     //ACP5 = RTC->ACP5;
     ACP5 = 6444; //(*(S_NVR_RTC1_P5_P4)>>16);
     
     //S_CLK_GetPCLKFreq() * 0.0002048  (0.0002048  this is a fixed value) //RTC->ACP6; 
     ACP6 = 5368 ; 
...
**************************************************************
*************************************************************/
void RTC_cal_update(uint32_t value)
{
    int16_t cal = 1;
    //uint32_t div_value;
   ...
   //PPMx10 = cal;
   if((uint16_t)cal != 0xffff && *(K1) == 22937)
   { 
     cal /= 10;
     cal += 0xffff;
     cal &= 0x3fff;
...
//#if (MicroController == Micro_V85XX)
   RTC_cal_update(0x3FD8);    //-40 value is a complement            //open it if we use an old version of VANGO
//#endif
