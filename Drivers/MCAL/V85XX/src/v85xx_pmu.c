...
//    return 1;
  ...
  timeout_pmu=0;
  while ((PMU->DSLEEPEN & PMU_DSLEEPEN_WKU)&&timeout_pmu<5000) //A_E add timeout
  {
  }
...
{
  /* Check parameters */
  assert_param(IS_PMU_INT(INTMask));
...
{
  /* Check parameters */
  assert_param(IS_PMU_INTFLAGR(INTMask));
...
{
  /* Check parameters */
  assert_param(IS_PMU_INTFLAGC(INTMask));
...
{
  /* Check parameters */
  assert_param(IS_GPIO_PINR(INTMask));  
...
{
  /* Check parameters */
  assert_param(IS_GPIO_PIN(INTMask)); 
...
void PMU_WakeUpPinConfig(uint32_t IOAx, uint32_t Wakeup_Event)
{
  ...
  /* Check parameters */
  assert_param(IS_GPIO_PINR(IOAx));   
  assert_param(IS_PMU_WAKEUP(Wakeup_Event));   
...
uint8_t PMU_EnterDSleep_LowPower(PMU_LowPWRTypeDef *InitStruct)
{
  ...
  /* Check parameters */  
  assert_param(IS_PMU_COMP1PWR(InitStruct->COMP1Power));
  assert_param(IS_PMU_COMP2PWR(InitStruct->COMP2Power));
  assert_param(IS_PMU_TADCPWR(InitStruct->TADCPower));
  assert_param(IS_PMU_BGPPWR(InitStruct->BGPPower));
  assert_param(IS_PMU_AVCCPWR(InitStruct->AVCCPower));
  assert_param(IS_PMU_LCDPWER(InitStruct->LCDPower));
  assert_param(IS_PMU_VDCINDET(InitStruct->VDCINDetector));
...
{
  /* Check parameters */
  assert_param(IS_PMU_RTCEVT(Wakeup_Event));
...
void PMU_PDNDSleepConfig(uint32_t VDCIN_PDNS, uint32_t VDD_PDNS)
{
  ...
  /* Check parameters */  
  assert_param(IS_PMU_VDCINPDNS(VDCIN_PDNS));
  assert_param(IS_PMU_VDDPDNS(VDD_PDNS));   
...
void PMU_VDDAlarmTHConfig(uint32_t PowerThreshold)
{
  ...
  /* Check parameters */ 
  assert_param(IS_PMU_PWTH(PowerThreshold));
...
{
  /* Check parameters */ 
  assert_param(IS_PMU_PWRSTA(PWRState));
...
void PMU_PowerSWTLevelConfig(uint32_t SWTLevel)
{
  ...
  /* Check parameters */ 
  assert_param(IS_PMU_PSWTLEVEL(SWTLevel));  
...
{
  /* Check parameters */ 
  assert_param(IS_PMU_BAT1DISC(BATDisc));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
...
void PMU_PWRDropDEBConfig(uint32_t Debounce)
{
  ...
  /* Check parameters */ 
  assert_param(IS_PMU_PWRDROP_DEB(Debounce)); 
...
{
  /* Check parameters */ 
  assert_param(IS_PMU_RSTSRC(RSTSource)); 
