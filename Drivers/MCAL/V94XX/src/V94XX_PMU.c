...
{
  return (PMU->IOAINTSTS&IOA_MASK);
}

/**
  * @brief  Get IOA interrupt status.
  * @param  INTMask:
  *             GPIO_Pin_0 ~ GPIO_Pin_1 or
  *             GPIO_Pin_3 ~ GPIO_Pin_9 or
...
{
  /* Check parameters */
  assert_parameters(IS_GPIOA_PINR(INTMask));  
...
/**
  ...
  * @param  IOAx:   GPIO_Pin_0 ~ GPIO_Pin_1 or
  *                 GPIO_Pin_3 ~ GPIO_Pin_9 or
  *                 GPIO_Pin_11 ~ GPIO_Pin_15
  *         Wakeup_Event:
  *                 IOA_DISABLE
...
void PMU_WakeUpPinConfig(uint32_t IOAx, uint32_t Wakeup_Event)
{
  ...
  /* Check parameters */
  assert_parameters(IS_GPIOA_PINR(IOAx));   
...
/**
  ...
  * @note   This function performs the following:
                ...
                VDD power control                    ON or OFF(optional)
...
uint8_t PMU_EnterDSleep_LowPower(PMU_LowPWRTypeDef *InitStruct)
{
  ...
  assert_parameters(IS_PMU_ADCBGPPWR(InitStruct->ADCBGPPower));
  ...
  /* Disable VDD output */
  ANA->REGF &= ~ANA_REGF_VDDO_EN;
  /* Power down ADC */
  ana_reg3_tmp &= ~ANA_REG3_ADCPDN;
  ANA->REG3 = ana_reg3_tmp;
  /* Power down Temp sensor */
  timeout_pmu=0;
  while ((ANA->ADCCTRL & ANA_ADCCTRL_MTRIG)&&timeout_pmu<5000); //A_E add timeout
  ...
  if ((InitStruct->AVCCPower) != PMU_VDCINDET_DISABLE)
  {
...
uint8_t PMU_EnterSleep_LowPower(PMU_LowPWRTypeDef *InitStruct, uint8_t mode)
{
  ...
  if (mode == 0)
   { 
  /* Set SLEEPDEEP bit of Cortex-M0 System Control Register 396uA */
...
/**
  * @brief  VDD Low Voltage detector power control.
  * @param  NewState:
  *             ENABLE
  *             DISABLE
  * @retval None
...
void PMU_VDDLVDetector_Cmd(uint32_t NewState)
{
  ...
  if (NewState == ENABLE)
    ANA->REG5 &= ~ANA_REG5_PD_VDDDET;
  else
    ANA->REG5 |= ANA_REG5_PD_VDDDET;
}

/**
  * @brief  Get VDD low power status.
  * @param  None
  * @retval low power status of VDD
  *           0: status not set, VDD is higher than 2.5V.
  *           1: status set, VDD is lower than 2.5V.
  */
...
{
  /* Check parameters */ 
  assert_parameters(IS_PMU_BAT2DISC(BATDisc));
  assert_parameters(IS_FUNCTIONAL_STATE(NewState));
