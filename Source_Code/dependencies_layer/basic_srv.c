...
/********************************** Headers ***********************************/
#include "dependencies_layer.h"/*header*/
#include "ctrl.h"
...
*/
uint8 itoa_generic (uint8* u8ptrbuf, uint64_t u32int_value)
{
  uint8_t index = 0,num_digits=0;
  uint64_t tempVariable = u32int_value;
  if (u32int_value == 0)
  {
    num_digits = 1;
  }
...
uint8 atoi_generic (uint8* u8ptrbuf, uint32* u32int_ptr , uint8_t len)
{
...
void vDelay_SW_RFID(uint32 u32LoopTimes)
{
#if 0
  /* Decrement nCount value */
...
void int_to_hexString_u8_u8(uint8_t* res , uint8_t x)
{
  uint8_t temp = 0;
  // = TO_HEX();
...
void hexString_to_int_u8_u8(uint8_t* res , uint8_t* x)
{
...
#if (defined(TENDER_1S)&& defined(ANSI_STACK))
   char *ee = (char *)(Source);
   *(DEST + 0) = *(ee + 3);
   *(DEST + 1) = *(ee + 2);
   *(DEST + 2) = *(ee + 1);
...
uint8_t check_card_date(uint8_t day, uint8_t month, uint8_t year)
{
  ...
  return (comp_date(day, month, year) <= RFID_CARD_VALIDATION_PERIOD);
#else
...
void Store_16BitData_Ansi(uint8_t *DEST, uint16_t* Source)
{
#if (MTR_NUM_OF_PHASE == 1)
   char *ee = (char *)(Source);
   *(DEST + 1) = *(ee + 0);
   *(DEST + 0) = *(ee + 1);  
#else
  ...
  *(DEST + 0) = *(ee + 0);
  *(DEST + 1) = *(ee + 1);
#endif//_MTR_NUM_OF_PH

}

void Store_8BitData(uint8_t *DEST, uint8_t* Source)
{
  char *ee = (char *)(Source);
  *DEST  = *ee;
}
...
void pack_digits(uint8_t *digits, uint8_t *packed, uint16_t len) 
{
  for (uint16_t i = 0; i < len / 2; i++) 
  {
    packed[i] = (digits[2 * i] << 4) | (digits[2 * i + 1] & 0x0F);
  }
} 

void unpack_digits(uint8_t *packed, uint8_t *digits, uint16_t len)
{
  for (uint16_t i = 0; i < len / 2; i++)
  {
    digits[2 * i] = ((packed[i] >> 4) & 0x0F)+'0';
    digits[2 * i + 1] = (packed[i] & 0x0F)+'0';
  }
}
