...
uint8_t V9380_InitClkFlag =0;
__IO uint32_t ana_reg3_tmp =0;
uint16_t timeout_clk;
/**
  * @brief  Initializes the CPU, AHB and APB buses clocks according to the specified 
  *         parameters in the CLK_ClkInitStruct.
  *
  * @note   This function performs the following:
  *             1. If want to switch AHB clock source, enable ADCBGP, enable 6.5M(Hz) RC,
...
void CLK_ClockConfig(CLK_InitTypeDef *CLK_ClkInitStruct)
{
  ...
  if ((CLK_ClkInitStruct->ClockType) & CLK_TYPE_AHBSRC)
  {
    ...
    else if (CLK_ClkInitStruct->AHBSource == CLK_AHBSEL_HSPLL)
    {
      ...
      timeout_clk=0;
      while (!(ANA->COMPOUT & ANA_COMPOUT_LOCKH)&&((timeout_clk++) <5000))
      {
      }
      /* clock source configuration */
      MISC2->CLKSEL = CLK_ClkInitStruct->AHBSource;  
    ...
    else if (CLK_ClkInitStruct->AHBSource == CLK_AHBSEL_LSPLL)
    {
      ...
      /* while loop until PLLL is lock */
      timeout_clk=0;
      while (!(ANA->COMPOUT & ANA_COMPOUT_LOCKL)&&((timeout_clk++) <5000))      //A_E timeout waiting 0
      {
      }
      /* clock source configuration */
      MISC2->CLKSEL = CLK_ClkInitStruct->AHBSource;  
...
uint32_t CLK_GetHCLKFreq(void)
{
  ...
  switch (ahb_clksrc)
  {
  /* AHB Clock source : 6.5M(Hz) RC */
  case MISC2_CLKSEL_CLKSEL_RCOH:
    hclk = 6553600 / ahb_div;
    break;
  
  /* AHB Clock source : 6.5M(Hz) XTAL */
...
/**
  * @brief  Get current external 6.5M(Hz) crystal status.
  *
  * @param  None
  *
  * @retval 6.5M(Hz) crystal status
  *           0: 6.5536M(Hz) crystal is absent.
  *           1: 6.5536M(Hz) crystal is present.
  */
...
/**
  * @brief  Get current external 32K(Hz) crystal status.
  *
  * @param  None
  *
  * @retval 32K(Hz) crystal status
  *           0: 32K(Hz) crystal is absent
  *           1: 32K(Hz) crystal is present.
  */
