...
static uint8_t first_init_ISO=0;
void ISO7816_Init(ISO7816_InitType *Init_Struct)
{
	...
	switch(Init_Struct->IFC){
     ...
     /*modify contact and RFID divison for max speed by test
     clk 26 ---> divider = ARM_ISO7816_APBD_8
     clk 13 ---> divider = ARM_ISO7816_APBD_4
     */
      if( first_init_ISO == 0)
      {
        first_init_ISO++;
      Drv_ISO7816[Init_Struct->IFC]->Control(ARM_ISO7816_SET_CLKDIV, ARM_ISO7816_APBD_8);      
...
/**
  * @brief  Read data.
  * @param  ISO7816: ISO7816
  * @retval The received data.
  */


uint8_t ISO7816_ReceiveData(uint8_t ifc, uint8_t *data)                         /*ADDCode_K*/  
{
	uint8_t ret = 0;
	switch(ifc){
		case 0:
