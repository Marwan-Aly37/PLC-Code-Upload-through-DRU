...
void UART_Init(UART_TypeDef *UARTx, UART_InitType *InitStruct)
{
  ...
  assert_param(IS_UART_WORDLEN(InitStruct->WordLen));
  assert_param(IS_UART_FIRSTBIT(InitStruct->FirstBit));
...
uint8_t UART_GetFlag(UART_TypeDef *UARTx, uint32_t FlagMask)
{
  /* Check parameters */
  assert_param(IS_UART_ALL_INSTANCE(UARTx));
  assert_param(IS_UART_FLAGR(FlagMask));
...
void UART_ClearFlag(UART_TypeDef *UARTx, uint32_t FlagMask)
{
  /* Check parameters */
  assert_param(IS_UART_ALL_INSTANCE(UARTx));
  assert_param(IS_UART_FLAGC(FlagMask));
...
void UART_INTConfig(UART_TypeDef *UARTx, uint32_t INTMask, uint8_t NewState)
{
  /* Check parameters */
  assert_param(IS_UART_ALL_INSTANCE(UARTx));
  assert_param(IS_UART_INT(INTMask));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
  
  if ((NewState == ENABLE)&&(UARTx == UART4))
  {
    UARTx->CTRL |= INTMask;
  }
  else
...
uint8_t UART_GetINTStatus(UART_TypeDef *UARTx, uint32_t INTMask)
{
  /* Check parameters */
  assert_param(IS_UART_ALL_INSTANCE(UARTx));
  assert_param(IS_UART_INTFLAGR(INTMask));
...
void UART_ClearINTStatus(UART_TypeDef *UARTx, uint32_t INTMask)
{
  /* Check parameters */
  assert_param(IS_UART_ALL_INSTANCE(UARTx));
  assert_param(IS_UART_INTFLAGC(INTMask));
